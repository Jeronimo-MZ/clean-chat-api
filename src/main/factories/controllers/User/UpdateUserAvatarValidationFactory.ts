...
export const makeUpdateUserAvatarValidation = (): Validation => {
    const fileValidations = [
        ...
        ...ValidationBuilder.field("mimetype").required().allowedMimetypes(["jpg", "png"]).build(),
    ];
    const objectIdValidator = new ObjectIdValidatorAdapter();

    return new ValidationComposite([
        ...ValidationBuilder.field("userId").required().objectId(objectIdValidator).build(),
        ...ValidationBuilder.field("file").required().object(fileValidations).build(),
    ]);
};
